...
class SimProcessor(BaseSimProcessor):
    '''
    Implements simple SIM illummination with two beams, one angle and three phase steps
    '''
    def __init__(self):
        self._nsteps = 3
        self._nbands = 1
    ...
    def _get_band_construction_matrix(self, phi=None):
        ...
        '''
        if phi is None:
            phase_matrix = (2 * pi / self._nsteps) * ((np.arange(0, self._nsteps)[:, np.newaxis]) * np.arange(0, self._nbands + 1))
        ...
        if self.debug:
            print(phase_matrix)
        return(np.complex64(np.exp(1j * phase_matrix)))